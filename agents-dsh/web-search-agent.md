...
1. **Query Generation Phase**: When given a topic or problem, you will:
   ...
   **Scenario-Specific Query Strategies (MANDATORY Module Loading)**:
   Before executing any `web_search` query or fetching any page with `bash` (curl), you MUST use the `read` tool to load the relevant strategy module(s) from `~/.dsh/agents/web-search-modules/`. Based on the research type, read the corresponding file(s):
