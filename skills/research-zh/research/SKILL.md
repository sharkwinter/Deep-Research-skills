...
用 AskUserQuestion 让用户选择调研模式（若从 topic 措辞已能明确判断，可直接建议默认项并说明，仍给用户改选机会）：
...
- 选 B → 执行下方 **模式 B** 流程（见"## 模式 B：深度叙事模式"），并遵循 `deep_narrative_playbook.md`。
...
### Step B1: 建工作区
- 创建 `./{topic_slug}/` 与 `./{topic_slug}/sources/`（用 Bash `mkdir -p`）。

### Step B2: 拆解调研角度
从话题本身拆 3~5 个**研究角度**（不是 item），每个角度可被证据回答、且能改变结论。常见角度：学术/理论、产业/参考实现、标准/规范、工程/开源生态、失败模式/批评。用 AskUserQuestion 与用户确认角度集与时间范围。

### Step B3: 每角度一个子代理（一手落盘）
每个角度启动 1 个子代理（背景运行；抓取密集可用 haiku，需高质量结论用 sonnet）。每个子代理的 brief **必须自包含**（不共享主控上下文），并强制：
- 用 WebSearch/WebFetch 找权威一手源（规范/论文/官方文档/维护活跃的仓库）；
...
主控**核验**每个子代理的落盘（`ls sources/`、查非零、抽查清单）——"报告 15 个源却只落 3 个"的情况真实发生过。若话题涉及本地代码/仓库，由主控自己直接勘察并写 `local_*.md` 源note（子代理只做网络侧，避免重复）。
...
- 交付：SendUserFile 把报告发给用户，并说明证据基座位置。
