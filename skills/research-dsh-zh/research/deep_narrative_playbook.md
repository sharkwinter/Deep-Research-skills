...
## 六、成本分层
抓取/落盘/写清单等机械工作与需高质量摘要/结论的角度，均由子代理完成；跨角度综合与成文由主模型完成。
（DSH 的 `subagent` 工具当前不支持按任务指定模型，子代理沿用会话默认模型；若部署支持子代理模型覆盖，可按"机械工作用轻量模型"分层。）

## 七、成文
`深度调研报告.md`：头部（日期/方法/证据基座份数+清单指针）→ `## 0 核心结论(TL;DR)` → 按角度分节正文（就地引用落盘源文件名）→ 结论/建议 → `## 参考文献`（指向各 SOURCES-*.md）。最后在回复中告知用户报告与 sources/ 的文件路径，完成交付。
