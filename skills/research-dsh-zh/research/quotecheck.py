#!/usr/bin/env python3
"""引文回源核查：报告里每段引文（英文与中文），必须能在 sources/ 里找到。
用法: quotecheck.py <报告.md> <sources 目录>
约定：英文引语用 "..."；中文原文引语用 “...”；中文转述用「」、法规与文件名用《》——转述与篇名不核验。"""
...
def is_chinese_prose(s):
    return len(re.findall(r'[\u4e00-\u9fff]', s))>=10    # ≥10个汉字才算引语（名词短语/法规名用《》不会命中）
cands=sorted(s for s in spans if is_english_prose(s) or is_chinese_prose(s))
bad=[]
for q in cands:
    minlen = 10 if not is_english_prose(q) else 15
    parts=[p for p in re.split(r'\.\.\.|…', q) if len(norm(p))>=minlen]  # 省略号切段，逐段核
    if all(norm(p) in srcn for p in (parts or [q])): continue
    bad.append(q)
en=sum(1 for c in cands if is_english_prose(c))
print(f"引文 {len(cands)} 条（英文 {en} / 中文 {len(cands)-en}） | 未回源 {len(bad)} 条")
